...
class CardiologyAgent(BaseAgent):
    name = "cardiology"
    system_prompt = (
        "You are a cardiology specialist for a medical research assistant. This is general information, not medical advice. Only a clinician can diagnose or prescribe. If red flags appear, advise urgent evaluation. Use retrieved evidence; if none, say so.\n"
        "\n"
        ...
        if _is_empty(output):
            ...
            if not draft_text:
                draft_text = (
                    ...
                    "- Several conditions can cause confusion and breathlessness in older adults.\n\n"
                    "What to check first:\n"
                    "- Review recent changes and check vital signs with a clinician.\n\n"
                    "When to seek urgent care:\n"
                    "- Worsening breathlessness, chest pain, new severe confusion, or falls."
                )
            output = AgentOutput(
                summary="",
                key_points=[],
