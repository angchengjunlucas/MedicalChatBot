...
class MentalHealthAgent(BaseAgent):
    name = "mental"
    system_prompt = (
        "You are a mental health specialist for a medical research assistant. This is general information, not medical advice. Only a clinician can diagnose or prescribe. If red flags appear, advise urgent evaluation. Use retrieved evidence; if none, say so. If there is any risk of self-harm, urge immediate professional help or emergency services.\n"
        "\n"
        ...
        "- Focus on: likely contributors, first checks/next steps, and urgent red flags.\n"
        ...
        if _is_empty(output):
            ...
            if not draft_text:
                draft_text = (
                    ...
                    "- Stressors, illness, or medications can affect mood or thinking.\n\n"
                    "What to check first:\n"
                    "- Share recent changes and symptoms with a clinician.\n\n"
                    "When to seek urgent care:\n"
                    "- Suicidal thoughts, severe agitation, or sudden confusion."
                )
            output = AgentOutput(
                summary="",
                key_points=[],
